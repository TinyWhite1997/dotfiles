...
const SKILLS_DIR = join(EXTENSION_DIR, "ai-toolkit", "revenuecat", "skills");
const DEFAULT_MCP_URL = "https://mcp.revenuecat.ai/mcp";
const MCP_URL = process.env.PI_REVENUECAT_MCP_URL || DEFAULT_MCP_URL;
...
function isEnabled(pi: ExtensionAPI): boolean {
  return pi.getFlag("revenuecat") === true;
}
...
function toolSchema(tool: McpTool): TSchema {
  ...
  // schema the TypeBox marker Pi needs; RevenueCat remains the source of truth
...
function stringifyContent(item: McpContent): string | undefined {
  ...
  if (item.type === "resource" && item.resource) {
    const heading = item.resource.uri
      ? `[RevenueCat resource: ${item.resource.uri}]`
      : "[RevenueCat resource]";
  ...
  if (item.type === "audio") {
    return `[RevenueCat returned ${item.mimeType || "audio"} content; audio data omitted]`;
  }
...
function formatResult(result: {
  ...
  const rawText = textParts.join("\n\n") || "RevenueCat returned no textual content.";
  ...
  if (truncated.truncated) {
    text +=
      `\n\n[RevenueCat output truncated: ${truncated.outputLines} of ${truncated.totalLines} lines` +
...
function resultErrorText(result: { content?: McpContent[] }): string {
  const text = (result.content ?? [])
    ...
    .filter((value): value is string => typeof value === "string")
    .join("\n");
  return text || "RevenueCat MCP tool call failed.";
}

export default function revenuecatExtension(pi: ExtensionAPI) {
  pi.registerFlag("revenuecat", {
    description: "Enable the RevenueCat MCP server and official RevenueCat skills",
    type: "boolean",
    default: false,
  });
  ...
  const connect = async (ctx: {
    ...
    connectPromise = (async () => {
      ctx.ui.setStatus("revenuecat-mcp", "RevenueCat: connecting…");
      stderrTail = "";
      ...
      const nextClient = new Client(
        { name: "pi-revenuecat", version: "1.0.0" },
        { capabilities: {} },
      );
      ...
      try {
        ...
        for (const tool of tools) {
          ...
          pi.registerTool({
            name: tool.name,
            label: `RevenueCat · ${tool.name}`,
            description: tool.description || `Call the RevenueCat MCP tool ${tool.name}`,
            parameters: toolSchema(tool),
            async execute(_toolCallId, params, signal) {
              const activeClient = client;
              if (!activeClient) {
                throw new Error(
                  "RevenueCat MCP is disconnected. Restart Pi with --revenuecat.",
                );
              }
              ...
              return {
                ...
                details: { server: "revenuecat", tool: tool.name },
              };
            },
          });
        }
        ...
        ctx.ui.setStatus("revenuecat-mcp", `RevenueCat: ${connectedToolCount} tools`);
        ctx.ui.notify(
          `RevenueCat MCP connected (${connectedToolCount} tools, official skills enabled).`,
          "info",
        );
        if (collisions.length > 0) {
          ctx.ui.notify(
            `Skipped RevenueCat tools whose names already exist: ${collisions.join(", ")}`,
            "warning",
          );
        }
      } catch (error) {
        ...
        throw new Error(
          logs ? `RevenueCat MCP connection failed: ${message}\n${logs}` : message,
        );
      } finally {
        ctx.ui.setStatus("revenuecat-mcp", undefined);
      }
    })();
  ...
  pi.registerCommand("revenuecat-status", {
    description: "Show whether the opt-in RevenueCat MCP integration is connected",
    handler: async (_args, ctx) => {
      if (!isEnabled(pi)) {
        ctx.ui.notify(
          "RevenueCat is disabled. Start Pi with: pi --revenuecat",
          "info",
        );
        return;
      }
      ctx.ui.notify(
        client
          ? `RevenueCat MCP is connected with ${connectedToolCount} tools.`
          : "RevenueCat is enabled but the MCP connection is unavailable.",
        client ? "info" : "warning",
      );
    },
  });
